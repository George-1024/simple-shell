...
void commandsave()
{	int i;
	memcpy(tempComms,ShellComms,8*MAX_COMMAND_NUM);

}
void commandrestore()
{ int i;

	memcpy(ShellComms,tempComms,8*MAX_COMMAND_NUM);

}
void menu(char argc,char **argv)
{
	int i,k;
	char *name;
	printf("\n");
  for(i = 0,k=1; i<MAX_COMMAND_NUM; i++ )
  {
		if (ShellComms[i].name!=0)
    {
			name = ShellComms[i].name;			
			printf("	%d. %s\n",k++,name);
		}
	}
...
{
	int i;
	ShellComms[0].CommandFunc = menu; 
 	ShellComms[0].name = "menu";
	for(i = 1; i<MAX_COMMAND_NUM; i++ )
	{ 
...
int register_command(void (*handler)(char argc,char **argv),char *name)
{  
	int i = 0;
	int pointer = 0;
	for(i =0;i<MAX_COMMAND_NUM;i++ ) 
 	{
 		...
 		{
			printf("\n\r This command function is already register as command %s!",ShellComms[i].name);
			return ;
		} 
	}
	for(i =0; i<MAX_COMMAND_NUM; i++ ) 
 	{
 		...
 		{
			printf("\n\r The command name %s is already registered !",ShellComms[i].name);
			return ;
		} 
	}
 	for(i = 0; i<MAX_COMMAND_NUM; i++ ) 
